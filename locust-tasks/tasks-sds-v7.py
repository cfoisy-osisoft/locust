...
@backoff.on_exception(backoff.expo, Sds503, max_tries=max_tries, jitter=backoff.full_jitter)
def get_interpolated_data(ocs_client, stream_id, start, end, count, stream_name, month_span):
...
def get_data(self):
    ...
    while True:
        ...
        if sema[stream_id].acquire(blocking=False) and not bad[stream_id]:
            break
        else:
            print(f"@@@@@@@@@@@@@@@@ discard {stream_id}|{stream_name}  bad={bad[stream_id]} @@@@@@@@@@@@@@@@@@@")
    ...
    try:
        result = get_interpolated_data(
                self.ocs_client, stream_id, start, end, self.event_count, stream_name, self.dv_month_span
        )
    except Sds503 as e:
        raise e 
    except SdsError as e:
        # 408s 
        if randrange(10) >= int(os.environ.get("BAD_CEILING", 10)):
            print(f"###################### bad stream {stream_id} | {stream_name} ##################")
            bad[stream_id] = True
        raise e
