...
for b, _, tag in all_tags:
    ...
    for s in streams:
        if s.Id not in dense_streams:
            all_streams.append((s.Id, s.Name))
            month_offset[s.Id] = 0
            month_span[s.Id] = int(os.environ.get("DV_SPAN_MONTH", "4"))
...
def get_data(self):
    ...
    try:
        ...
        if month_span[stream_id] > 1:
            ...
            span = int(math.ceil(month_span[stream_id] / 2.0))
            month_span[stream_id] = span
            print(
                f"@@@@@@@@@@@@@@@@@@@@@@ new span: {stream_id}|{stream_name}: from {old_span} -> {span}"
            )
        else:
            print(
